...
var prefix = ayarlar.prefix;
  ...
  if (!user) return message.channel.send("Sunucudan yasaklamak istediğiniz kullanıcıyı etiketlemelisiniz; \`${prefix}ban @Kullanıcı Reklam\` ");
  ...
  if (user.position > message.member.roles.highest.position) return message.channel.send(`Bu kullanıcının senin rollerinden/rolünden daha yüksek rolleri/rolü var.`);
    ...
    if (!user) return message.channel.send(`Etiketlediğin kullanıcıyı sunucuda bulamadım.:x:`)
    let member = message.guild.member(user)
    if (!member) return message.channel.send(`Etiketlediğin kullanıcıyı sunucuda bulamadım.:x:`)
